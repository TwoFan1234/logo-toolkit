...
class BatchVideoToolWidget(QWidget):
    ...
    def _build_endcard_page(self) -> QWidget:
        ...
        self.endcard_overlap_spin.setValue(1.5)
        self.endcard_overlap_spin.valueChanged.connect(self._refresh_selected_metadata)

        self.endcard_crossfade_spin = QDoubleSpinBox()
        self.endcard_crossfade_spin.setDecimals(2)
        self.endcard_crossfade_spin.setRange(0.0, 600.0)
        self.endcard_crossfade_spin.setSingleStep(0.1)
        self.endcard_crossfade_spin.setSuffix(" 秒")
        self.endcard_crossfade_spin.setValue(0.5)
        self.endcard_crossfade_spin.valueChanged.connect(self._refresh_selected_metadata)

        self.endcard_alpha_mode_combo = QComboBox()
        self.endcard_alpha_mode_combo.addItem("PR兼容（去黑边）", VideoEndCardAlphaMode.PREMIERE_COMPAT)
        self.endcard_alpha_mode_combo.addItem("直接叠加", VideoEndCardAlphaMode.DIRECT)

        form.addRow("重叠时长", self.endcard_overlap_spin)
        form.addRow("音频交叉淡变", self.endcard_crossfade_spin)
        form.addRow("透明通道处理", self.endcard_alpha_mode_combo)
        ...
        return VideoBatchConfig(
            ...
            endcard=VideoEndCardSettings(
                ...
                audio_crossfade_seconds=(
                    self.endcard_crossfade_spin.value()
                    if hasattr(self, "endcard_crossfade_spin")
                    else 0.5
                ),
                alpha_mode=(
                    self.current_endcard_alpha_mode()
                    if hasattr(self, "endcard_alpha_mode_combo")
                    else VideoEndCardAlphaMode.PREMIERE_COMPAT
                ),
            ),
        )
    ...
    def current_endcard_alpha_mode(self) -> VideoEndCardAlphaMode:
        if not hasattr(self, "endcard_alpha_mode_combo"):
            return VideoEndCardAlphaMode.PREMIERE_COMPAT
        current = self.endcard_alpha_mode_combo.currentData()
        if isinstance(current, VideoEndCardAlphaMode):
            return current
        return VideoEndCardAlphaMode(str(current or VideoEndCardAlphaMode.PREMIERE_COMPAT.value))
